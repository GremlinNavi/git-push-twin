# PEMDAS twin identifiers and PowerShell integration
...
## Git identity versus PEMDAS identity

The proposal deliberately separates several kinds of identity:

```text
PEMDAS expression -> human-readable twin/member identity
...
```text
PEMDAS ID  -> which oversight context
Git hash   -> which code
...
10. Cryptographic signatures may attribute approval, but PEMDAS identifiers themselves do not authenticate people.

## Summary

The proposed PEMDAS layer gives `twin` three related capabilities:
...
```text
PEMDAS -> semantic relationship and human-readable provenance
